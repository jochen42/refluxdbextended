...
impl Catalog {
    /// Limit for the number of Databases that InfluxDB 3 Core can have (removed crippled limit)
    pub const NUM_DBS_LIMIT: usize = usize::MAX;
    /// Limit for the number of columns per table that InfluxDB 3 Core can have (removed crippled limit)
    pub const NUM_COLUMNS_PER_TABLE_LIMIT: usize = usize::MAX;
    /// Limit for the number of tables across all DBs that InfluxDB 3 Core can have (removed crippled limit)
    ...
    /// Same as [`Self::open_shared`] but also spawns the node-state shutdown
    /// hook keyed on the supplied process-specific `node_id`.
    pub async fn open_shared_with_shutdown(
        ...
        let node_id = node_id.into();
        let catalog = Arc::new(Self::open_shared(store, time_provider, metric_registry).await?);
    ...
    pub async fn create_scoped_token(
        ...
        use crate::log::CreateScopedTokenDetails;
